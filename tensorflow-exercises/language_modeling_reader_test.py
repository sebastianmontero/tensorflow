...
class PtbReaderTest(tf.test.TestCase):
    ...
    def testPtbRawData(self):
        ...
        for suffix in 'train', 'valid', 'test':
            filename = os.path.join(tmpdir, 'ptb.{}.txt'.format(suffix))
            with tf.gfile.GFile(filename, 'w') as fh:
                fh.write(self._string_data)
            
        output = language_modeling_reader.ptb_raw_data(tmpdir)
    ...
    def testPtbProducer(self):
        ...
        batch_size = 3
        num_steps = 2
        x, y = language_modeling_reader.ptb_producer(raw_data, batch_size, num_steps)
